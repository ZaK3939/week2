...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    var poolBalance = await token.balanceOf(tornadoPool.address)
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    const bobEAddress = '0xDeaD00000000000000000000000000000000BEEf'
    const bobChangeUtxo = new Utxo({ amount: 0, keypair: bobKeypair })
    await transaction({
      tornadoPool,
      inputs: [bobReceiveUtxo],
      outputs: [bobChangeUtxo],
      recipient: bobEAddress,
    })
    var bobAddressBalance = await token.balanceOf(bobEAddress)
    expect(bobAddressBalance).to.be.equal(utils.parseEther('0.06'))
    await transaction({
      tornadoPool,
    ...
    const recipientBalance = await token.balanceOf(bobEAddress)
